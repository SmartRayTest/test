               ...
               , uint8_t*       aLiveImage )
{
    ...
    printf( "live image  originX: %d  height: %d  width: %d\n", aOriginX, aHeight, aWidth );
...
int main()
{
    ...
    if ( status )
    {
        printf( "Registering live image callback (SR_API_RegisterLiveImageCB)\n" );

        const int32_t registerStatusCode = SR_API_RegisterLiveImageCB( onLiveImage );
        const bool    isRegistered       = ( SUCCESS <= registerStatusCode );
        if ( ! isRegistered )
        {
            printf( "SR_API_RegisterLiveImageCB failed with error: %d\n", registerStatusCode );
        }
        status = isRegistered;
    }
    ...
    if ( status )
    {
        ...
        const ImageAquisitionType  imageAcquisitionType    = ImageAquisitionType_LiveImage;
