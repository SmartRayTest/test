...
int main()
{
    ...
    if ( status )
    {
        printf( "Setting exposure time (SR_API_SetExposureTime)\n" );

        const int32_t exposureIndex         = 0;
        const int32_t exposureTimeMicroSec  = 100;
        const int32_t exposureStatusCode = SR_API_SetExposureTime( &sensor, exposureIndex, exposureTimeMicroSec );
        const bool    isexposureSet      = ( SUCCESS <= exposureStatusCode );
        if ( ! isexposureSet )
        {
