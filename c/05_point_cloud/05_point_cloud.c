                    ...
                    , void*          aExtData )
{
    ...
    sPointCloudCounter += (uint32_t) aNumProfile;

    printf( "Point cloud callback: received %4d profiles, overall: %u\n", aNumProfile, sPointCloudCounter );
...
int main()
{
    ...
    if ( status )
    {
        printf( "Registering point cloud callback (SR_API_RegisterPointCloudCB)\n" );

        const int32_t registerStatusCode = SR_API_RegisterPointCloudCB( onPointCloudData );
        const bool    isRegistered       = ( SUCCESS <= registerStatusCode );
        if ( ! isRegistered )
        {
            printf( "SR_API_RegisterPointCloudCB failed with error: %d\n", registerStatusCode );
        }
        status = isRegistered;
    }
    ...
    if ( status )
    {
        ...
        const ImageAquisitionType  imageAcquisitionType    = ImageAquisitionType_PointCloud;
