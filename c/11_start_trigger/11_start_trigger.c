...
int main()
{
    ...
    if ( status )
    {
        printf( "Setting start trigger (SR_API_SetStartTrigger)\n" );

        const bool    enable                = true;
        const int32_t setStartStatusCode    = SR_API_SetStartTrigger( &sensor, StartTriggerSource_Input0, enable, TriggerEdgeMode_RisingEdge );
        const bool    isStartSet            = ( SUCCESS <= setStartStatusCode );
        if ( ! isStartSet )
        {
            printf( "SR_API_SetStartTrigger failed with error: %d\n", setStartStatusCode );
        }
        status = isStartSet;
    }
