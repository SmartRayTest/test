             ...
             , void*          aExtData )
{
    ...
    sPILCounter += (uint32_t) aHeight;

    printf( "PIL callback: received %4d profiles, overall: %u\n", aHeight, sPILCounter );
...
int main()
{
    ...
    if ( status )
    {
        printf( "Registering PIL image callback (SR_API_RegisterPilImageCB)\n" );

        const int32_t registerStatusCode = SR_API_RegisterPilImageCB( onPILData );
        const bool    isRegistered       = ( SUCCESS <= registerStatusCode );
        if ( ! isRegistered )
        {
            printf( "SR_API_RegisterPilImageCB failed with error: %d\n", registerStatusCode );
        }
        status = isRegistered;
    }
    ...
    if ( status )
    {
        ...
        const ImageAquisitionType  imageAcquisitionType    = ImageAquisitionType_ProfileIntensityLaserLineThickness;
