...
int main()
{
    ...
    if ( status )
    {
        printf( "Setting region of interest (SR_API_SetROI)\n" );

        const int32_t offsetX       = 0;
        const int32_t offsetY       = 0;
        const int32_t width         = 256;
        const int32_t height        = 256;
        const int32_t roiStatusCode = SR_API_SetROI( &sensor, offsetX, width, offsetY, height );
        const bool    isRoiSet      = ( SUCCESS <= roiStatusCode );
        if ( ! isRoiSet )
        {
            printf( "SR_API_SetROI failed with error: %d\n", roiStatusCode );
        }
        status = isRoiSet;
    }
