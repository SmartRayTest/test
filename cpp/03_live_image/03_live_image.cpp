...
static uint32_t  sLiveImageCounter = 0;

int onLiveImage( SRSensor*      aSensor
               ...
               , uint8_t*       aLiveImage )
{
    ...
    std::cout << "live image  originX: " << aOriginX
              << "  height: " << aHeight
              << "  width: "  << aWidth
              << std::endl;
...
int main()
{
    ...
    if ( status )
    {
        std::cout << "Registering live image callback" << std::endl;

        const int32_t registerStatus = SensorManager::registerLiveImageCallback( onLiveImage );
        const bool    isRegistered   = ( SUCCESS <= registerStatus );
        if ( ! isRegistered )
        {
            std::cout << "SensorManager::registerLiveImageCallback() failed with error: " << registerStatus << std::endl;
        }
        status = isRegistered;
    }
    ...
    if ( status )
    {
        ...
        const int32_t setTypeStatus = sensor->setImageAcquisitionType( ImageAquisitionType_LiveImage );
