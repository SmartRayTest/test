...
int main()
{
    ...
    if ( status )
    {
        ...
        const int32_t exposureIndex         = 0;
        const int32_t exposureTimeMicroSec  = 100;
        const int32_t exposureStatus        = sensor->setExposureTime( exposureIndex, exposureTimeMicroSec );
        const bool    isExposureSet         = ( SUCCESS <= exposureStatus );
        if ( ! isExposureSet )
        {
            std::cout << "Sensor::setExposureTime() failed with error: " << exposureStatus << std::endl;
        }
