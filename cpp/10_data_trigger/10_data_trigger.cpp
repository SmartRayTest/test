...
int main()
{
    ...
    if ( status )
    {
        std::cout << "Setting data trigger mode" << std::endl;

        const int32_t setTriggerModeStatus = sensor->setDataTriggerMode( DataTriggerMode_Internal );
        const bool    isTriggerModeSet     = ( SUCCESS <= setTriggerModeStatus );
        if ( ! isTriggerModeSet )
        {
            std::cout << "Sensor::setDataTriggerMode() failed with error: " << setTriggerModeStatus << std::endl;
        }
        status = isTriggerModeSet;
    }

    if ( status )
    {
        std::cout << "Setting data trigger frequency" << std::endl;

        const uint16_t frequencyHz   = 500;
        const int32_t  setFreqStatus = sensor->setDataTriggerInternalFrequency( frequencyHz );
        const bool     isFreqSet     = ( SUCCESS <= setFreqStatus );
        if ( ! isFreqSet )
        {
            std::cout << "Sensor::setDataTriggerInternalFrequency() failed with error: " << setFreqStatus << std::endl;
        }
