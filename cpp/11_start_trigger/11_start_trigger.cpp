...
int main()
{
    ...
    if ( status )
    {
        std::cout << "Setting start trigger" << std::endl;

        const bool    enable         = true;
        const int32_t setStartStatus = sensor->setStartTrigger( StartTriggerSource_Input0, enable, TriggerEdgeMode_RisingEdge );
        const bool    isStartSet     = ( SUCCESS <= setStartStatus );
        if ( ! isStartSet )
        {
            std::cout << "Sensor::setStartTrigger() failed with error: " << setStartStatus << std::endl;
        }
