...
static uint32_t  sPILCounter = 0;

int onPILData( SRSensor*      aSensor
             , ImageDataType  aImageType
             , int            aOriginX
             , int            aHeight
             , int            aWidth
             , uint16_t*      aProfileImage
             ...
             , void*          aExtData )
{
    ...
    sPILCounter += static_cast<uint32_t>( aHeight );

    std::cout << "PIL callback: received " << aHeight << " profiles, "
              << "overall: " << sPILCounter << std::endl;
...
int main()
{
    ...
    if ( status )
    {
        std::cout << "Registering PIL image callback" << std::endl;

        const int32_t registerStatus = SensorManager::registerPilImageCallback( onPILData );
        const bool    isRegistered   = ( SUCCESS <= registerStatus );
        if ( ! isRegistered )
        {
            std::cout << "SensorManager::registerPilImageCallback() failed with error: " << registerStatus << std::endl;
        }
        status = isRegistered;
    }
    ...
    if ( status )
    {
        ...
        const int32_t setTypeStatus = sensor->setImageAcquisitionType( ImageAquisitionType_ProfileIntensityLaserLineThickness );
