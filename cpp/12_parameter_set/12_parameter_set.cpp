...
int main()
{
    ...
    if ( status )
    {
        std::cout << "Loading parameter set file" << std::endl;

        const int32_t loadStatus = sensor->loadParametersFromFile( "parameter_set.json" );
        const bool    isLoaded   = ( SUCCESS <= loadStatus );
        if ( ! isLoaded )
        {
            std::cout << "Sensor::loadParametersFromFile() failed with error: " << loadStatus << std::endl;
        }
        status = isLoaded;
    }
    ...
    if ( status )
    {
        std::cout << "Sending parameter set to sensor" << std::endl;

        const int32_t sendStatus = sensor->sendParametersToSensor();
        const bool    isSent     = ( SUCCESS <= sendStatus );
        if ( ! isSent )
        {
            std::cout << "Sensor::sendParametersToSensor() failed with error: " << sendStatus << std::endl;
        }
